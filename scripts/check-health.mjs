...
const base = scriptConfig.apiUrl;
try {
  ...
  if (!response.ok || body.status !== 'ok' || body.database !== 'connected' || body.cache !== 'connected')
    throw new Error(`API is not healthy (database=${body.database}, cache=${body.cache})`);
  console.log(`Health check passed: status=${body.status}, database=${body.database}, cache=${body.cache}`);
} catch (error) {
  console.error(`Blocking: health check failed (${error instanceof Error ? error.message : 'unknown error'}).`);
  process.exit(1);
}
