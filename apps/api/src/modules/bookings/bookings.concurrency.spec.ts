...
describe('BookingsService concurrency protection', () => {
  it('rejects a simultaneous booking when the Redis lock is already held', async () => {
    ...
    // No Setting rows configured, so every rule resolves to its own default.
    const settings = { numeric: jest.fn().mockImplementation((_key: string, fallback: number) => Promise.resolve(fallback)) } as any;
