...
export class AvailabilityController {
  ...
  @Public() @Get(':teacherId/slots') slots(@Param('teacherId') id:string,@Query('from') from:string,@Query('to') to:string,@Query('type') type?:SlotType){return this.s.slots(id,new Date(from),new Date(to),type==='trial'?'trial':'regular');}
