...
export class MatchingService {
  ...
  async create(userId: string, data: MatchInput) {
    ...
    if (!language) throw badRequest('MATCH_LANGUAGE_INVALID');
    const now = new Date(),
      to = new Date(now.getTime() + 31 * 86_400_000);
    ...
    const teachers = await this.db.teacher.findMany({
      where: {
        ...
        availabilityRules: {
          some: { active: true, ...(data.suitableDays.length ? { weekday: { in: data.suitableDays } } : {}) },
        },
    ...
    for (const teacher of teachers) {
      ...
      const suitableSlots = slots.filter((slot) => {
        ...
        const weekday = Number(
          new Intl.DateTimeFormat('en-US', { timeZone: data.timezone, weekday: 'short' })
            .formatToParts(local)
            .find((part) => part.type === 'weekday')?.value
            ? ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].indexOf(
                new Intl.DateTimeFormat('en-US', { timeZone: data.timezone, weekday: 'short' }).format(local),
              )
            : -1,
        );
        ...
        const hour = Number(
          new Intl.DateTimeFormat('en-US', { timeZone: data.timezone, hour: '2-digit', hourCycle: 'h23' }).format(
            local,
          ),
        );
      ...
      const skillHits = [...new Set([...teacher.specialties, ...(languageLink?.specialties ?? [])])].filter((skill) =>
      ...
      candidates.push({
        ...
        reasons: { fa: reasonsFa, en: reasonsEn },
        audit: {
          ...
          price,
          compatibleSlots: suitableSlots.length,
          firstSlot: suitableSlots[0],
        },
      });
    }
    ...
    return this.db.matchingSession.create({
      data: {
        ...
        targetBand: data.targetBand ?? 0,
        currentBand: data.currentBand,
        examDate: data.examDate ? new Date(data.examDate) : undefined,
        weakSkills: data.weakSkills,
        maxTrialPrice: data.budget,
        availability: data.availability ?? { suitableDays: data.suitableDays, preferredTime: data.preferredTime },
        suitableDays: data.suitableDays,
        preferredTime: data.preferredTime,
        preferredTeacherGender: data.preferredTeacherGender,
        trialRequired: data.trialRequired,
          ...
          include: {
            teacher: {
              select: {
                ...
                lessonDuration: true,
                specialties: true,
                languageLinks: { where: { languageId: data.languageId }, include: { language: true } },
              },
            },
          },
          orderBy: { rank: 'asc' },
  ...
  history(userId: string) {
    return this.db.matchingSession.findMany({
      ...
      include: {
        language: true,
        recommendations: {
          include: { teacher: { include: { languageLinks: { include: { language: true } } } } },
          orderBy: { rank: 'asc' },
        },
      },
