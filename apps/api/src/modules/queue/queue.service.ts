...
export class QueueService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    this.worker = new Worker(
      'notifications',
      async (job) => {
        if (job.name === 'booking-expiration') {
          await this.expireBooking(job.data.bookingId);
          return;
        }
        ...
        for (const user of [reminder.booking.student, reminder.booking.teacher.user]) {
          ...
          try {
            ...
            if (this.cfg.KAVENEGAR_API_KEY) {
              const r = await fetch(
                `${this.cfg.KAVENEGAR_API_BASE}/v1/${this.cfg.KAVENEGAR_API_KEY}/verify/lookup.json?receptor=${encodeURIComponent(user.phone)}&token=${encodeURIComponent(when.date)}&token2=${encodeURIComponent(when.time)}&template=${encodeURIComponent(this.cfg.KAVENEGAR_REMINDER_TEMPLATE)}`,
                { method: 'POST' },
              );
              response = (await r.json()) as object;
              if (!r.ok) throw new Error('Kavenegar delivery failed');
  ...
  async scheduleExpiration(bookingId: string, expiresAt: Date) {
    await this.queue.add(
      'booking-expiration',
      { bookingId },
      {
        jobId: `expiration-${bookingId}`,
