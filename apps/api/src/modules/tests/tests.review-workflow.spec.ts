...
describe('TestsService descriptive review workflow', () => {
  it('returns the complete queue item after an examiner claims an answer', async () => {
    ...
    expect(db.testAnswer.update).toHaveBeenCalledWith(
  ...
  it('removes the final answer from pending and approves the attempt transactionally', async () => {
    ...
    const result = await service.reviewAnswer('examiner-1', {
      answerId: 'answer-1',
      band: 7,
      criteria: { coherence: 7 },
      feedbackFa: 'بازخورد فارسی',
    ...
    expect(tx.testAnswer.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'answer-1' },
        data: expect.objectContaining({ reviewStatus: 'APPROVED', reviewerId: 'examiner-1', finalScore: 7 }),
      }),
    );
    expect(tx.testAttempt.update).toHaveBeenCalledWith({
      where: { id: 'attempt-1' },
      data: { status: 'APPROVED', overallBand: 7 },
    });
    expect(tx.notification.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ userId: 'student-1', type: 'TEST_RESULT_READY' }) }),
    );
  ...
  it('keeps the attempt under review when another descriptive answer remains', async () => {
    ...
    await expect(
      service.reviewAnswer('examiner-1', {
        answerId: 'answer-1',
        band: 6.5,
        criteria: {},
        feedbackFa: 'الف',
...
describe('TestsService revision resubmission', () => {
  it('returns only approved answers in the reviewed queue', async () => {
    ...
    expect(tx.testAnswer.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('resubmits only a needs-revision answer and clears the previous review state', async () => {
    ...
    const db = {
      testAttempt: {
        findFirst: jest
          .fn()
          .mockResolvedValue({ id: 'attempt-1', testId: 'test-1', status: 'UNDER_REVIEW', expiresAt: new Date(0) }),
      },
      question: { findMany: jest.fn().mockResolvedValue([{ id: 'question-1', type: 'essay' }]) },
      testAnswer: {
        findMany: jest.fn().mockResolvedValue([{ questionId: 'question-1', reviewStatus: 'NEEDS_REVISION' }]),
      },
      $transaction: jest.fn((callback) => callback(tx)),
    } as any;
    const service = new TestsService(db, {} as any);

    await service.save('student-1', 'attempt-1', [
      { questionId: 'question-1', textValue: 'Revised answer', flagged: false },
    ]);

    expect(tx.testAnswer.update).toHaveBeenCalledWith({
      where: { attemptId_questionId: { attemptId: 'attempt-1', questionId: 'question-1' } },
      data: expect.objectContaining({
        textValue: 'Revised answer',
  ...
  it('rejects editing an approved answer while the attempt is under review', async () => {
    ...
    await expect(
      service.save('student-1', 'attempt-1', [
        { questionId: 'question-1', textValue: 'Changed answer', flagged: false },
      ]),
