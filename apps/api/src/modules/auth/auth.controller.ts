...
@Controller('auth')export class AuthController{constructor(private auth:AuthService){}private set(res:Response,token:string){res.cookie('refresh_token',token,{httpOnly:true,secure:config().NODE_ENV==='production',sameSite:'lax',path:'/api/auth',maxAge:30*864e5})}@Public()@RateLimit(OTP_SEND_LIMIT)@Post('otp/request')request(@Body()d:RequestOtpDto,@Ip()ip:string){return this.auth.requestOtp(d.phone,ip)}@Public()@RateLimit(OTP_SEND_LIMIT)@Post('otp/resend')resend(@Body()d:RequestOtpDto,@Ip()ip:string){return this.auth.requestOtp(d.phone,ip)}@Public()@RateLimit(OTP_VERIFY_LIMIT)@Post('otp/verify')async verify(@Body()d:VerifyOtpDto,@Req()req:Request,@Res({passthrough:true})res:Response){const out=await this.auth.verifyOtp(d.challengeId,d.phone,d.code,{ip:req.ip,userAgent:req.headers['user-agent']});this.set(res,out.refreshToken);const{refreshToken,...safe}=out;return safe}@Public()@RateLimit(REFRESH_LIMIT)@Post('refresh')async refresh(@Req()req:Request,@Res({passthrough:true})res:Response){const out=await this.auth.refresh(req.cookies?.refresh_token,{ip:req.ip,userAgent:req.headers['user-agent']});this.set(res,out.refreshToken);const{refreshToken,...safe}=out;return safe}@Public()@RateLimit(LOGOUT_LIMIT)@Post('logout')async logout(@Req()req:Request,@Res({passthrough:true})res:Response){await this.auth.logout(req.cookies?.refresh_token??'');res.clearCookie('refresh_token',{path:'/api/auth'});return{ok:true}}}
