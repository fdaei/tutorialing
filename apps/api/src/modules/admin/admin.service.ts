...
import { forbidden, TokenRevocationService } from '../../common';
...
export class AdminService {
      ...
      include: {
        ...
        attempts: {
          ...
          select: {
            ...
            startedAt: true,
            submittedAt: true,
            test: { select: { titleFa: true, titleEn: true } },
          },
        },
        payments: {
          take: 5,
          orderBy: { createdAt: 'desc' },
          select: { purpose: true, amount: true, status: true, createdAt: true },
        },
        tickets: {
          take: 5,
          orderBy: { updatedAt: 'desc' },
          select: { subject: true, status: true, priority: true, updatedAt: true },
        },
        learningPlans: {
          take: 5,
          orderBy: { updatedAt: 'desc' },
  ...
  async createUser(
    ...
    await this.db.auditLog.create({
      data: { actorId, action: 'user.created', entity: 'User', entityId: user.id, after: { phone: user.phone, roles } },
    });
    return user;
  }
  ...
  setSetting(key: string, value: unknown, isPublic: boolean) {
    return this.db.setting.upsert({
      ...
      update: { value: value as object, public: isPublic },
    });
  }
