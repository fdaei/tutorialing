...
describe('DashboardStatsService', () => {
  it('serves all metrics with exactly two read queries', async () => {
    const statsQuery = Promise.resolve(stat);
    const activityQuery = Promise.resolve([{ id: 'audit-1' }]);
    const db = {
      dashboardStat: { findUniqueOrThrow: jest.fn().mockReturnValue(statsQuery) },
      auditLog: { findMany: jest.fn().mockReturnValue(activityQuery) },
      $transaction: jest.fn().mockResolvedValue([stat, [{ id: 'audit-1' }]]),
    };

    const result = await new DashboardStatsService(db as never).dashboard();

    expect(db.$transaction).toHaveBeenCalledWith([statsQuery, activityQuery]);
    expect(db.dashboardStat.findUniqueOrThrow).toHaveBeenCalledTimes(1);
  ...
  it('rebuilds only after acquiring the cross-instance advisory lock', async () => {
    ...
    await expect(new DashboardStatsService(db as never).reconcile()).resolves.toBe(true);
  ...
  it('does not run a competing rebuild when another replica owns the lock', async () => {
    ...
    await expect(new DashboardStatsService(db as never).reconcile()).resolves.toBe(false);
  ...
  it('refuses to serialize a metric after it exceeds the safe JSON integer range', async () => {
    ...
    await expect(new DashboardStatsService(db as never).dashboard()).rejects.toThrow('revenue exceeds JSON safe integer');
  });
});
