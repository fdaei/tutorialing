  ...
  return (
    <main className="grid min-h-screen place-items-center p-6">
      <div className="text-center">
        ...
        <h1 className="mt-7 text-4xl font-black">{fa ? 'پرداخت تأیید شد' : 'Payment confirmed'}</h1>
        <p className="mt-4 text-muted">
          {fa
            ? 'رزرو یا بسته شما در پنل قابل مشاهده است.'
            : 'Your booking or package is now available in the dashboard.'}
        </p>
        <Link
          href={localePath('/dashboard', locale)}
          className="brand-gradient mt-7 inline-block rounded-xl px-7 py-4 font-black text-white"
        >
          {fa ? 'رفتن به پنل' : 'Go to dashboard'}
        </Link>
      </div>
    </main>
  );
