  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#f7f8fc] p-6">
      <div className="max-w-md rounded-4xl border hairline bg-white p-8 text-center shadow-soft">
        <p className="text-sm font-black text-purple">{fa ? 'درگاه توسعه' : 'Development gateway'}</p>
        <h1 className="mt-4 text-3xl font-black">{fa ? 'شبیه‌ساز تأیید پرداخت' : 'Payment verification simulator'}</h1>
        ...
        {verify.isError && (
          <p role="alert" className="mt-4 text-red-700">
            {fa ? 'تأیید ناموفق بود.' : 'Verification failed.'}
          </p>
        )}
      </div>
    </main>
