...
export default function Auth() {
  ...
  async function send() {
    ...
    if (!/^09\d{9}$/.test(phone)) {
      setError(
        new ApiError(400, {
          code: 'PHONE_INVALID',
          message: fa ? 'شماره موبایل معتبر نیست.' : 'The mobile number is invalid.',
          fieldErrors: {
            phone: fa
              ? 'شماره باید با 09 شروع شود و دقیقاً 11 رقم باشد؛ مانند 09123456789.'
              : 'It must start with 09 and contain exactly 11 digits, such as 09123456789.',
          },
        }),
      );
      return;
  ...
  async function verify() {
    ...
    if (code.length !== 6) {
      setError(
        new ApiError(400, {
          code: 'OTP_INVALID',
          message: fa ? 'کد تأیید کامل نیست.' : 'The verification code is incomplete.',
          fieldErrors: { code: fa ? 'هر 6 رقم پیامک‌شده را وارد کنید.' : 'Enter all six digits from the SMS.' },
        }),
      );
      return;
    }
    ...
    try {
      const response = await publicApi<{ accessToken: string; user?: PanelIdentity }>('/auth/otp/verify', {
        method: 'POST',
        credentials: 'include',
        body: JSON.stringify({ phone, challengeId: challenge, code }),
      });
  ...
  const c = fa
    ? {
        ...
        codeLead: `کد شش‌رقمی ارسال‌شده به ${phone} را وارد کنید.`,
        phone: 'شماره موبایل',
        continue: 'ارسال کد',
    ...
    : {
        ...
        codeLead: `Enter the six-digit code sent to ${phone}.`,
        phone: 'Mobile number',
        continue: 'Send code',
  ...
  return (
    <main className="relative grid min-h-screen place-items-center bg-[#f6f7fc] p-3 md:p-8">
      <LanguageSwitcher
        className={`absolute top-4 z-20 rounded-xl border hairline bg-white px-3 py-2 shadow-sm ${fa ? 'left-4' : 'right-4'}`}
      />
      <div
        className={`grid w-full max-w-[1120px] overflow-hidden rounded-[30px] border hairline bg-white shadow-[0_28px_90px_rgba(24,29,72,.12)] lg:grid-cols-2 ${fa ? 'lg:[direction:rtl]' : ''}`}
      >
        <section className="relative hidden min-h-[690px] overflow-hidden bg-[#eef1fb] lg:block">
          <Image
            src="/images/lingospeak-student.png"
            alt={fa ? 'زبان‌آموز در حال مطالعه' : 'Student studying'}
            fill
          ...
          <Link
            href={p('/')}
            className={`absolute top-8 flex items-center gap-2 text-lg font-black text-white drop-shadow ${fa ? 'right-8' : 'left-8'}`}
          >
          ...
          <div className="absolute inset-x-8 bottom-8 rounded-[24px] border border-white/25 bg-navy/68 p-6 text-white backdrop-blur-md">
            <h2 className="text-3xl font-black leading-tight">
              {fa ? 'هر زبان، مسیر مناسب خودش را دارد.' : 'Every language deserves the right learning path.'}
            </h2>
        ...
        <section className="flex min-h-[620px] items-center p-7 sm:p-12 lg:p-16">
          <div className="w-full" dir={fa ? 'rtl' : 'ltr'}>
            <Link href={p('/')} className="mb-12 flex items-center gap-2 lg:hidden">
              <span className="brand-gradient grid size-10 place-items-center rounded-full text-white">
                <MessageCircle />
              </span>
              <strong className="latin text-xl">LingoSpeak</strong>
            </Link>
            <p className="text-sm font-bold text-purple">{step === 'phone' ? c.badge : c.verify}</p>
            <h1 className="mt-3 text-4xl font-black tracking-[-.035em]">{step === 'phone' ? c.title : c.verify}</h1>
            <p className="mt-4 text-sm leading-7 text-muted">{step === 'phone' ? c.lead : c.codeLead}</p>
            {message && (
            ...
            {step === 'phone' ? (
              <label className="mt-8 block">
                <span className="mb-2.5 block text-sm font-black">{c.phone}</span>
                <div
                  dir="ltr"
                  className={`flex overflow-hidden rounded-[14px] border bg-white focus-within:ring-4 focus-within:ring-violet/10 ${field ? 'border-red-400' : 'border-[#dce0eb] focus-within:border-purple'}`}
                >
                  <span className="flex items-center gap-2 border-r hairline bg-[#fafbfe] px-4 text-sm">🇮🇷 +98</span>
                  <input
                    inputMode="tel"
                    autoComplete="tel"
                    value={phone}
                ...
                <div dir="ltr" className="grid grid-cols-6 gap-2" onPaste={paste}>
                  {digits.map((digit, index) => (
                    <input
                      ...
                      aria-label={`${fa ? 'رقم' : 'Digit'} ${index + 1}`}
                      inputMode="numeric"
                      autoComplete={index === 0 ? 'one-time-code' : 'off'}
                      value={digit}
                      onChange={(event) => changeDigit(index, event.target.value)}
                      onKeyDown={(event) => key(index, event)}
                      className={`aspect-square min-w-0 rounded-xl border text-center text-2xl font-black outline-none focus:ring-4 focus:ring-violet/10 ${field ? 'border-red-400' : 'hairline focus:border-purple'}`}
                    />
                  ))}
                </div>
                {field && <span className="mt-2 block text-xs text-red-700">{field}</span>}
            ...
            <button
              ...
              className="brand-gradient brand-glow mt-7 flex w-full items-center justify-center gap-3 rounded-[14px] py-4 font-black text-white transition hover:-translate-y-0.5 disabled:opacity-50"
            >
            ...
            {step === 'otp' && (
              <div className="mt-5 flex justify-between text-sm">
                ...
                <button disabled={wait > 0 || busy} onClick={send} className="disabled:text-muted">
                  {wait ? `${c.resend} (${new Intl.NumberFormat(fa ? 'fa-IR' : 'en-US').format(wait)})` : c.resend}
                </button>
              </div>
            )}
