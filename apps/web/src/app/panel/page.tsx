...
export default function PanelRouter(){
  ...
  const me=useQuery({queryKey:['panel-me'],queryFn:()=>api<{roles:string[]}>('/users/me'),retry:false});
  useEffect(()=>{
    if(me.data){const roles=me.data.roles??[],target=roles.some(role=>['ADMIN','STAFF'].includes(role))?'/admin':roles.includes('TEACHER')?'/teacher-panel':'/dashboard';router.replace(p(target))}
