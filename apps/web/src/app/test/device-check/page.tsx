...
export default function DeviceCheck() {
  ...
  const me = useQuery({ queryKey: ['device-check-me'], queryFn: () => api('/users/me'), retry: false }),
    tests = useQuery({
      queryKey: ['tests', selectedTestId],
      queryFn: () => publicApi<{ id: string; titleFa: string; titleEn: string }[]>('/tests'),
    });
  const start = useMutation({
    mutationFn: () =>
      api<{ id: string }>('/tests/attempts', {
        method: 'POST',
        body: JSON.stringify({
          testId: (selectedTestId && tests.data?.find((test) => test.id === selectedTestId)?.id) || tests.data?.[0]?.id,
        }),
      }),
    onSuccess: (r) => router.push(p(`/test/session?attempt=${r.id}`)),
    onError: (error) => {
      if (error instanceof ApiError && error.status === 401) router.replace(p(`/auth?next=${p('/test/device-check')}`));
    },
  });
  useEffect(() => {
    if (me.error instanceof ApiError && me.error.status === 401)
      router.replace(p(`/auth?next=${p('/test/device-check')}`));
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-6 py-14">
      ...
      <button
        disabled={!me.data || !online || mic !== 'ok' || !tests.data?.length || start.isPending}
...
function CheckRow({ icon, title, detail, ok }: { icon: React.ReactNode; title: string; detail: string; ok: boolean }) {
  return (
    <div className="flex items-center gap-5 rounded-3xl border hairline bg-white p-6">
      {icon}
      <div className="flex-1">
      ...
      {ok ? <CheckCircle2 className="text-green-700" /> : <XCircle className="text-red-700" />}
    </div>
  );
}
