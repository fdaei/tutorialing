...
export function StudentTests() {
  ...
  return (
    <>
      <PageHeading
        title="آزمون‌ها"
        description="آزمون‌های فعال، روند پیشرفت و نتیجه‌های تأییدشده را یک‌جا دنبال کنید."
      />
      {q.isLoading && (
      ...
      {q.isError && (
        <div role="alert" className="rounded-2xl bg-red-50 p-5 text-red-700">
          {apiMessage(q.error, 'دریافت آزمون‌ها ناموفق بود.')}{' '}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        {q.data?.map((a) => {
          ...
          const progress = done ? 100 : review ? 90 : 35;
          const language =
            typeof a.test.language === 'string'
              ? a.test.language
              : a.test.language?.nameFa || a.test.language?.nativeName || a.test.language?.nameEn;
          return (
            <article key={a.id} className="panel-card p-5 md:p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <span className="text-xs font-bold text-indigo-600">
                    {language || 'آزمون زبان'} · {a.test.level ?? 'تعیین سطح'}
                  </span>
                  <h2 className="mt-2 text-lg font-black">{a.test.titleFa || a.test.titleEn}</h2>
                </div>
                <span className={`status-pill ${done ? 'status-success' : review ? 'status-warning' : 'status-info'}`}>
                  {labels[a.status] ?? a.status}
                </span>
              </div>
              <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
                <Meta label="تاریخ شروع" value={jalali(a.createdAt)} />
                <Meta label="آخرین فعالیت" value={jalali(a.updatedAt)} />
                <Meta label="تاریخ پایان" value={jalali(a.submittedAt ?? a.expiresAt)} />
                <Meta label="مدت آزمون" value="حدود ۶۰ دقیقه" />
              </div>
              <div className="mt-5">
                <div className="mb-2 flex justify-between text-xs">
                  <span className="text-muted">پیشرفت آزمون</span>
                  <b>{progress.toLocaleString('fa-IR')}٪</b>
                </div>
              ...
              <div className="mt-5 flex items-center justify-between">
                <span className="flex items-center gap-2 text-xs text-muted">
                  <Clock3 size={15} />
                  {a.overallBand != null
                    ? `نمره نهایی: ${a.overallBand.toLocaleString('fa-IR')}`
                    : 'آخرین ذخیره خودکار ثبت شده'}
                </span>
                {review ? (
                  <button disabled className="secondary-button opacity-60">
                    در انتظار بررسی
                  </button>
                ) : (
                  <Link
