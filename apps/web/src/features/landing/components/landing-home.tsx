    ...
    >
      ...
      <main>
        {config.sections
          ...
          .map((section) => <LandingSection key={section.id} {...props} section={section} />)}
      </main>
      ...
      <div className="landing-container landing-hero-grid">
        ...
        <div className="landing-hero-visual">
          <div className="landing-hero-image">
            ...
            {config.hero.overlay > 0 && <span className="landing-image-overlay" style={{ opacity: config.hero.overlay }} />}
          </div>
  ...
  return (
    <section className="landing-section" style={sectionStyle(section, config.theme)}>
      <div className="landing-container">
        <SectionIntro eyebrow={t(config.languages.eyebrow)} title={t(config.languages.title)} description={t(config.languages.description)} />
      ...
      <div className="landing-container">
        ...
        <div className="landing-benefit-grid">
          {config.benefits.items.map((item, index) => {
            ...
            return (
              <article key={`${item.title.en}-${index}`} className="landing-benefit-item">
                <span className="landing-benefit-icon"><Icon size={20} /></span>
      ...
      <div className="landing-container">
        <div className="landing-placement" style={{ backgroundColor: config.placement.backgroundColor }}>
          ...
          <div className="landing-placement-art">
            ...
            <div className="landing-level-stack"><span>A1</span><span>B1</span><span>C1</span></div>
            <div className="landing-placement-stamp"><Target size={17} /><span>CEFR<br /><strong>A1—C2</strong></span></div>
          </div>
        </div>
      </div>
    </section>
...
function CoursesSection({ config, locale, courses, section }: LandingRenderProps) {
  ...
  return (
    <section className="landing-section" style={sectionStyle(section, config.theme)}>
      <div className="landing-container">
        <SectionIntro eyebrow={t(config.courses.eyebrow)} title={t(config.courses.title)} description={t(config.courses.description)} />
              ...
              return (
                <Link href={path(`/courses/${course.slug}`)} key={course.slug} className="landing-course-card">
                  <div className="landing-course-image">
                    {course.image ? <img src={course.image} alt="" /> : <span className="landing-course-placeholder"><BookGlyph /></span>}
...
function BlogSection({ config, locale, posts, section }: LandingRenderProps) {
  ...
  return (
    <section className="landing-section landing-section-tint" style={sectionStyle(section, config.theme)}>
      <div className="landing-container">
        <SectionIntro eyebrow={t(config.blog.eyebrow)} title={t(config.blog.title)} description={t(config.blog.description)} />
        <div className="landing-blog-grid">
          {postItems.map((post) => (
            <Link href={path(`/blog/${post.slug}`)} className="landing-blog-card" key={post.slug}>
              <div className="landing-blog-image">{post.coverImage ? <img src={post.coverImage} alt="" /> : <span><Sparkles size={22} /></span>}</div>
        ...
        <div className="landing-faq-list">
          {config.faq.items.map((item, index) => (
            <details key={`${item.question.en}-${index}`} open={index === 0}>
              <summary>{t(item.question)} <ChevronDown size={18} /></summary>
              <p>{t(item.answer)}</p>
            </details>
          ))}
        </div>
...
function BookGlyph() {
  return <span aria-hidden="true" className="landing-book-glyph">LS</span>;
}
