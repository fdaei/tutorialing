...
export function AudioRecorder({ value, onUploaded }: Props) {
  ...
  const recorder = useRef<MediaRecorder | null>(null),
    ...
    pendingFileId = useRef<string | null>(null),
    objectUrl = useRef<string | null>(null);
  const [status, setStatus] = useState<'idle' | 'recording' | 'uploading' | 'ready' | 'error'>(
    value ? 'uploading' : 'idle',
  );
  ...
  useEffect(() => {
    ...
    setStatus('uploading');
    setError('');
    api<Download>(`/files/${value}/download`)
      .then((result) => {
        if (!cancelled) {
  ...
  async function start() {
    try {
      ...
      const supported = ['audio/webm;codecs=opus', 'audio/webm', 'audio/mp4', 'audio/ogg;codecs=opus'].find((type) =>
      ...
      media.onstop = () => {
        ...
        const blob = new Blob(chunks.current, { type: (media.mimeType || supported || 'audio/webm').split(';')[0] });
  ...
  async function retry() {
    ...
    try {
      if (pendingFileId.current) await attach(pendingFileId.current);
        ...
        const result = await api<Download>(`/files/${value}/download`);
        replaceUrl(result.url);
        setStatus('ready');
      } else setStatus('idle');
    } catch {
  ...
  return (
    <div className="mt-5 rounded-2xl border hairline bg-lavender/30 p-5" aria-live="polite">
