...
function Data({ data, empty, locale }: { data: unknown; empty: string; locale: Locale }) {
  ...
  if (!dataRows.length)
    return <div className="mt-7 rounded-3xl border border-dashed hairline p-10 text-center text-muted">{empty}</div>;
  return (
    <div className="mt-7 grid gap-4">
      {dataRows.map((raw, i) => {
        const row = isRow(raw) ? raw : { value: raw },
          entries = displayEntries(row, locale);
        return (
          <article key={text(row.id) || i} className="overflow-hidden rounded-3xl border hairline bg-white p-6">
            {entries.length ? (
              <dl className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {entries.slice(0, 15).map((entry) => (
                  <div key={entry.key}>
                    <dt className="text-xs text-muted">{entry.label}</dt>
                    <dd className="mt-1 break-words font-bold">{format(entry.value, entry.key, locale)}</dd>
                  </div>
                ))}
              </dl>
            ) : (
              <p className="text-center text-sm text-muted">{empty}</p>
            )}
          </article>
        );
      })}
...
function displayEntries(row: Row, locale: Locale): DisplayEntry[] {
  ...
  const person = (key: 'user' | 'student', faLabel: string, enLabel: string) => {
    const item = isRow(row[key]) ? (row[key] as Row) : undefined;
    if (item)
      add(
        `${key}Name`,
  ...
  person('user', 'کاربر', 'User');
  person('student', 'زبان‌آموز', 'Student');
...
const labelsFa: Record<string, string> = {
  ...
  activeTeachers: 'مدرسان فعال',
  pendingTeachers: 'مدرسان در انتظار',
  testAttempts: 'تلاش‌های آزمون',
  pendingReviews: 'آزمون‌های در انتظار بررسی',
  bookings: 'رزروها',
  payments: 'پرداخت‌ها',
  payouts: 'تسویه‌ها',
  openTickets: 'تیکت‌های باز',
  revenue: 'درآمد',
  walletLiability: 'تعهد کیف پول',
};
...
function format(value: unknown, key: string, locale: Locale) {
  ...
  if (
    typeof value === 'number' &&
    ['amount', 'price', 'balance', 'revenue', 'walletLiability'].some((part) =>
