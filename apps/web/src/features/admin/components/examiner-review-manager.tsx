...
type QueueAnswer = {
  ...
  reviewBand?: number;
  reviewCriteria?: Record<string, number>;
  feedbackFa?: string;
  feedbackEn?: string;
  reviewedAt?: string;
  reviewer?: { name?: string };
  question: { prompt: { fa?: string; en?: string }; type: string; section: { skill: string; title: string } };
  attempt: {
    submittedAt: string;
    ...
    test: {
      titleFa: string;
      titleEn: string;
      language: { nameFa: string; nameEn: string; nativeName: string; flag?: string };
    };
  };
};
type Queue = { items: QueueAnswer[]; pagination: { page: number; pageSize: number; total: number; pages: number } };
const tabs = [
...
export function ExaminerReviewManager() {
  const { locale } = useTranslations(),
    ...
    [active, setActive] = useState<QueueAnswer | null>(null);
  const queue = useQuery({
    queryKey: ['examiner-queue', tab, page],
    queryFn: () => api<Queue>(`/examiner/tests/queue?status=${tab}&page=${page}&pageSize=20`),
  });
  const claim = useMutation({
    mutationFn: (id: string) => api<QueueAnswer>(`/examiner/tests/answers/${id}/claim`, { method: 'POST' }),
    onSuccess: (answer) => {
      setActive((current) =>
        current?.id === answer.id
          ? {
              ...current,
              ...answer,
              question: answer.question ?? current.question,
              attempt: answer.attempt ?? current.attempt,
            }
          : answer,
      );
  ...
  return (
    <div className="grid gap-6">
      <section className="rounded-3xl border hairline bg-white p-5">
      ...
      <div className="grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
        <section className="rounded-3xl border hairline bg-white p-4">
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <button
              disabled={page >= (queue.data?.pagination.pages ?? 1)}
              onClick={() => setPage((value) => value + 1)}
              className="rounded-xl border hairline px-4 py-2 disabled:opacity-30"
            >
...
function ReviewEditor({
  ...
  const [band, setBand] = useState(answer.reviewBand ?? 6.5),
    [feedbackFa, setFeedbackFa] = useState(answer.feedbackFa ?? ''),
    [feedbackEn, setFeedbackEn] = useState(answer.feedbackEn ?? ''),
    [status, setStatus] = useState<'APPROVED' | 'NEEDS_REVISION'>(
      answer.reviewStatus === 'NEEDS_REVISION' ? 'NEEDS_REVISION' : 'APPROVED',
    ),
    [criteria, setCriteria] = useState<Record<string, number>>(
      answer.reviewCriteria ?? defaultCriteria(answer.question.section.skill),
    );
  const review = useMutation({
    mutationFn: () =>
      api('/examiner/tests/answers/review', {
        method: 'POST',
        body: JSON.stringify({ answerId: answer.id, band, criteria, feedbackFa, feedbackEn, status }),
      }),
    onSuccess: done,
  });
  const canEdit = answer.reviewStatus === 'IN_REVIEW' || tab === 'in_review';
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Status status={answer.reviewStatus} fa={fa} />
      </div>
      <div className="mt-6 rounded-2xl bg-[#f7f8fc] p-5">
        ...
        {answer.textValue && (
          <div className="mt-4 whitespace-pre-wrap rounded-xl bg-white p-5 leading-8">{answer.textValue}</div>
        )}
        {answer.fileId && <ReviewAudio fileId={answer.fileId} fa={fa} />}
      </div>
        ...
        <div className="mt-7">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {Object.entries(criteria).map(([key, value]) => (
              <label key={key}>
                <span className="mb-2 block text-xs text-muted">{criterionLabel(key, fa)}</span>
                <input
                  type="number"
                  min={0}
                  max={9}
                  step={0.5}
                  value={value}
                  onChange={(event) => setCriteria((current) => ({ ...current, [key]: Number(event.target.value) }))}
                  className="input"
                />
              </label>
            ))}
          ...
          <label className="mt-5 block">
            ...
            <textarea
              dir="ltr"
              value={feedbackEn}
              onChange={(event) => setFeedbackEn(event.target.value)}
              className="input min-h-32"
              placeholder="Explain the issue, why it matters, and how the learner can improve."
            />
          </label>
          {review.isError && (
...
function ReviewAudio({ fileId, fa }: { fileId: string; fa: boolean }) {
  const query = useQuery({
    queryKey: ['review-audio', fileId],
    queryFn: () => api<{ url: string }>(`/files/${fileId}/download`),
  });
  return (
...
function Status({ status, fa }: { status: ReviewStatus; fa: boolean }) {
  const mapFa = {
      PENDING: 'در انتظار',
    ...
    mapEn = { PENDING: 'Pending', IN_REVIEW: 'In review', APPROVED: 'Approved', NEEDS_REVISION: 'Needs revision' };
  return (
    <span
...
function defaultCriteria(skill: string): Record<string, number> {
  return skill === 'speaking'
    ? { fluency: 0, lexicalResource: 0, grammar: 0, pronunciation: 0 }
    : { taskAchievement: 0, coherence: 0, lexicalResource: 0, grammar: 0 };
}
function criterionLabel(key: string, fa: boolean) {
  const faMap: Record<string, string> = {
    fluency: 'روانی و انسجام',
    lexicalResource: 'دامنه واژگان',
    grammar: 'دقت و تنوع گرامری',
    pronunciation: 'تلفظ',
...
function Empty({ text }: { text: string }) {
  return (
    <div className="grid min-h-44 place-items-center rounded-2xl border border-dashed hairline p-8 text-center text-muted">
      {text}
    </div>
  );
}
