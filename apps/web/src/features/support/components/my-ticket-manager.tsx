  ...
  return (
    <section className="mt-7">
      <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-black">{fa ? 'پشتیبانی و تیکت‌ها' : 'Support tickets'}</h1>
      ...
      <div className="panel-card mb-4 grid gap-3 p-3 sm:grid-cols-[1fr_190px]">
        <label className="flex items-center gap-2 rounded-xl border hairline px-3">
        ...
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="input py-3">
      ...
      {list.isError && (
        <ErrorBox
          message={apiMessage(list.error, fa ? 'تیکت‌های شما دریافت نشد.' : 'Could not load your tickets.')}
          retry={() => list.refetch()}
        />
      )}
      {list.data && !list.data.items.length && (
        <div className="rounded-3xl border border-dashed hairline p-10 text-center text-muted">
          {fa ? 'هنوز تیکتی ثبت نکرده‌اید.' : 'You have not created a ticket yet.'}
        </div>
      )}
          ...
          <div className="grid content-start gap-3">
            {visible.map((ticket) => (
              <button
                ...
                className={`rounded-2xl border p-4 text-start ${selectedId === ticket.id ? 'border-purple bg-lavender/40' : 'hairline bg-white'}`}
              >
          ...
          <div className="rounded-3xl border hairline bg-white p-5">
            {!selectedId && (
              <p className="py-16 text-center text-muted">
                {fa ? 'برای مشاهده گفتگو یک تیکت را انتخاب کنید.' : 'Select a ticket to view the conversation.'}
              </p>
            )}
                  ...
                  <div className="mt-6">
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3">
                      <button
                        type="button"
                        disabled
                        title="ضمیمه پس از انتخاب فایل به attachmentId تبدیل می‌شود"
                        className="secondary-button opacity-60"
                      >
                        <Paperclip size={17} />
                        ضمیمه فایل
                      </button>
                      {reply.isError && (
                        <p role="alert" className="text-sm text-red-700">
                          {apiMessage(reply.error, fa ? 'پاسخ ارسال نشد.' : 'Could not send the reply.')}
                        </p>
                      )}
                      <button
                      ...
                      >
                        {reply.isPending ? (fa ? 'در حال ارسال…' : 'Sending…') : fa ? 'ارسال پاسخ' : 'Send reply'}
                      </button>
                    </div>
                  </div>
                )}
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-black">ایجاد تیکت جدید</h2>
              <button type="button" onClick={() => setCreating(false)} aria-label="بستن">
                <X />
              </button>
            </div>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <label className="sm:col-span-2">
                ...
                <input required minLength={3} maxLength={160} name="subject" className="input" />
              </label>
              <label>
                <span className="mb-2 block text-sm font-bold">دسته‌بندی</span>
              ...
              <label>
                <span className="mb-2 block text-sm font-bold">اولویت</span>
              ...
              <label className="sm:col-span-2">
                ...
                <textarea required minLength={2} maxLength={5000} name="body" className="input min-h-32" />
              </label>
            </div>
            {create.isError && (
              <p className="mt-3 text-sm text-red-600">{apiMessage(create.error, 'تیکت ایجاد نشد.')}</p>
            )}
            <div className="mt-5 flex justify-end gap-2">
              <button type="button" onClick={() => setCreating(false)} className="secondary-button">
                انصراف
              </button>
              <button disabled={create.isPending} className="primary-button">
...
function Status({ value, fa }: { value: string; fa: boolean }) {
  ...
  return (
    <span className="shrink-0 rounded-full bg-lavender px-3 py-1 text-xs font-black text-purple">
      {fa ? (labels[value] ?? value) : value.replaceAll('_', ' ')}
    </span>
  );
}
