...
type TicketListItem = {
  ...
  replies: { body: string; author: { name?: string }; createdAt: string }[];
...
type TicketDetail = Omit<TicketListItem, 'replies'> & {
  createdAt: string;
  replies: Reply[];
  statusHistory: {
    fromStatus?: string;
  ...
  assignmentHistory: { createdAt: string; note?: string; actor: { name?: string } }[];
};
const statuses = ['OPEN', 'IN_PROGRESS', 'WAITING_USER', 'WAITING_SUPPORT', 'RESOLVED', 'CLOSED'];
...
export function TicketManager() {
  const { locale } = useTranslations(),
    ...
    [user, setUser] = useState<Option | null>(null),
    [assigneeFilter, setAssigneeFilter] = useState<Option | null>(null);
  const me = useQuery({
      queryKey: ['panel-me'],
      queryFn: () => api<{ permissions: string[] }>('/users/me'),
      retry: false,
    }),
    canManage = Array.isArray(me.data?.permissions) && me.data.permissions.includes('tickets.manage');
  ...
  if (assigneeFilter) params.set('assignedToId', assigneeFilter.id);
  const list = useQuery({
    queryKey: ['tickets', scope, status, priority, search, page, user?.id, assigneeFilter?.id],
    queryFn: () => api<TicketPage>(`/support/tickets?${params.toString()}`),
  });
  ...
  return (
    <div className="grid gap-6">
      <section className="rounded-3xl border hairline bg-white p-5">
        <div className="grid gap-3 lg:grid-cols-[1.2fr_repeat(3,.7fr)]">
          <label className="flex items-center gap-2 rounded-xl border hairline px-4">
            <Search size={17} />
            <input
              value={search}
              onChange={(event) => {
      ...
      <div className="grid gap-6 xl:grid-cols-[410px_minmax(0,1fr)]">
        <section className="rounded-3xl border hairline bg-white p-4">
            ...
            <div className="mt-4 grid gap-3">
              {list.data.items.map((ticket) => (
                <button
                  ...
                  className={`rounded-2xl border p-4 text-start ${selectedId === ticket.id ? 'border-purple bg-lavender/35' : 'hairline hover:border-purple'}`}
                >
                  ...
                  <p className="mt-2 text-sm text-muted">
                    {ticket.user.name || ticket.user.phone} · {ticket.category} · {priorityLabel(ticket.priority, fa)}
                  </p>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <button
              disabled={page >= (list.data?.pagination.pages ?? 1)}
              onClick={() => setPage((value) => value + 1)}
              className="rounded-xl border hairline px-4 py-2 disabled:opacity-30"
            >
...
function TicketDetailView({
  ...
  const [status, setStatus] = useState(ticket.status),
    [assignee, setAssignee] = useState<Option | null>(
      ticket.assignedTo
        ? {
            id: ticket.assignedTo.id,
            label: ticket.assignedTo.name || ticket.assignedTo.phone,
            description: ticket.assignedTo.phone,
          }
        : null,
    ),
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: refresh,
    onError: () => setStatus(ticket.status),
  });
  const assignment = useMutation({
    mutationFn: () =>
      api(`/support/tickets/${ticket.id}/assignment`, {
        method: 'PATCH',
        body: JSON.stringify({ assignedToId: assignee?.id ?? null, note: note || undefined }),
      }),
    onSuccess: refresh,
  });
  const reply = useMutation({
    mutationFn: () =>
      api(`/support/tickets/${ticket.id}/replies`, { method: 'POST', body: JSON.stringify({ body, internal }) }),
    onSuccess: async () => {
      setBody('');
      setInternal(false);
      await refresh();
    ...
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm font-bold text-purple">
            {ticket.category} · {priorityLabel(ticket.priority, fa)}
          </p>
          <h2 className="mt-2 text-2xl font-black">{ticket.subject}</h2>
        </div>
        <TicketStatus value={ticket.status} fa={fa} />
      </div>
      ...
      <div className="mt-8 space-y-4">
        {ticket.replies.map((message) => (
          <Message key={message.id} message={message} fa={fa} />
        ))}
      </div>
      <div className="mt-7 rounded-2xl border hairline p-4">
        <textarea
          value={body}
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <strong className="flex items-center gap-2">
          ...
          {kind} · {message.author.name || message.authorRole}
        </strong>
        <span className="text-xs text-muted">{format(message.createdAt, fa)}</span>
      </div>
...
function TicketStatus({ value, fa }: { value: string; fa: boolean }) {
...
function Empty({ text }: { text: string }) {
  return (
    <div className="grid min-h-40 place-items-center rounded-2xl border border-dashed hairline p-8 text-center text-muted">
      {text}
    </div>
  );
}
