...
type Block = { id: string; startsAt: string; endsAt: string; reason?: string; adminCreated: boolean };
type Availability = { timezone: string; rules: Rule[]; overrides: Override[]; blocks: Block[] };
...
export function TeacherAvailabilityManager() {
  ...
  const [rules, setRules] = useState<Rule[]>([]),
    [blockStart, setBlockStart] = useState<Date | null>(null),
    [blockEnd, setBlockEnd] = useState<Date | null>(null),
    [blockReason, setBlockReason] = useState(''),
    [overrideDate, setOverrideDate] = useState<Date | null>(null),
    [overrideAvailable, setOverrideAvailable] = useState(false),
    [overrideStart, setOverrideStart] = useState('09:00'),
    [overrideEnd, setOverrideEnd] = useState('17:00'),
  ...
  const removeBlock = useMutation({
    mutationFn: (id: string) => api(`/availability/me/blocks/${id}`, { method: 'DELETE' }),
    onSuccess: invalidate,
  });
  const addOverride = useMutation({
    mutationFn: () => {
      ...
      return api('/availability/me/overrides', {
        method: 'POST',
        body: JSON.stringify({
          date: dateOnlyIso(overrideDate),
          available: overrideAvailable,
          startMinute: overrideAvailable ? sh * 60 + sm : undefined,
          endMinute: overrideAvailable ? eh * 60 + em : undefined,
  ...
  const removeOverride = useMutation({
    mutationFn: (id: string) => api(`/availability/me/overrides/${id}`, { method: 'DELETE' }),
    onSuccess: invalidate,
  });
  function addRule() {
    setRules((current) => [
      ...current,
      {
        ...
        startMinute: 9 * 60,
        endMinute: 17 * 60,
        timezone: query.data?.timezone || 'Asia/Tehran',
        lessonDuration: 60,
        breakMinutes: 15,
      },
    ]);
  ...
  return (
    <div className="grid gap-6">
      <section className="rounded-3xl border hairline bg-white p-6">
          ...
          {rules.length ? (
            rules.map((rule, index) => (
              ...
              >
                ...
                <TimeInput value={rule.startMinute} onChange={(startMinute) => updateRule(index, { startMinute })} />
                <TimeInput value={rule.endMinute} onChange={(endMinute) => updateRule(index, { endMinute })} />
                <NumberInput
        ...
        {saveRules.isError && <InlineError error={saveRules.error} field="rules" fa={fa} />}
        <button
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-3xl border hairline bg-white p-6">
          ...
          <div className="mt-5 grid gap-4">
            ...
            <label className="flex items-center gap-3 rounded-xl border hairline p-4">
              <input
                type="checkbox"
                checked={overrideAvailable}
                onChange={(event) => setOverrideAvailable(event.target.checked)}
          ...
          <div className="mt-6 grid gap-3">
            {query.data?.overrides.map((item) => (
              <article key={item.id} className="flex items-center justify-between gap-3 rounded-2xl bg-[#f7f8fc] p-4">
                <div>
                  <strong>{formatDate(item.date, fa)}</strong>
                ...
                <button onClick={() => removeOverride.mutate(item.id)} className="text-red-600">
                  <Trash2 size={18} />
                </button>
              </article>
            ))}
        ...
        <section className="rounded-3xl border hairline bg-white p-6">
          ...
          <div className="mt-5 grid gap-4">
            ...
            <JalaliDateTimePicker value={blockEnd} onChange={setBlockEnd} minDate={blockStart ?? new Date()} />
            <input
              value={blockReason}
...
function TimeInput({ value, onChange }: { value: number; onChange: (value: number) => void }) {
  return (
    <input
      ...
      onChange={(event) => {
        const [h = 0, m = 0] = event.target.value.split(':').map(Number);
        onChange(h * 60 + m);
      }}
      className="input"
    />
...
function InlineError({ error, field, fa }: { error: unknown; field: string; fa: boolean }) {
...
function ErrorBox({ message, retry }: { message: string; retry: () => void }) {
  return (
    <div role="alert" className="rounded-3xl bg-red-50 p-6 text-red-800">
      {message}{' '}
