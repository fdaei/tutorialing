  ...
  return (
    <>
      <button
        ...
        aria-label={`پخش ویدیوی معرفی ${teacherName}`}
      ...
      >
        <div className="flex items-center justify-between border-b hairline p-5">
          <h2 id="teacher-video-title" className="font-black">
            ویدیوی معرفی {teacherName}
          </h2>
          <button type="button" onClick={() => setOpen(false)} aria-label="بستن ویدیو" className="grid size-10 place-items-center rounded-xl hover:bg-canvas">
            <X aria-hidden="true" />
          </button>
        </div>
        <div className="bg-navy p-4 sm:p-6">
          {video.isPending ? (
            <div aria-label="در حال دریافت ویدیو" className="skeleton aspect-video rounded-2xl" />
          ) : video.isError ? (
            <div role="alert" className="grid aspect-video place-items-center content-center gap-4 rounded-2xl bg-white p-6 text-center">
              <strong>ویدیو دریافت نشد</strong>
              <p className="text-sm text-muted">اتصال خود را بررسی کنید و دوباره تلاش کنید.</p>
