...
export function Header(){
 ...
 const links:[string,string][]=[[p('/teachers'),t('teachers')],[p('/placement'),t('placement')],[p('/matching'),t('matching')],[p('/teacher-apply'),locale==='fa'?'مدرس شوید':'Become a teacher'],[p('/about'),locale==='fa'?'درباره ما':'About us']];
 ...
 */
function TrustSeals(){
 const snippet=process.env.NEXT_PUBLIC_ENAMAD_HTML;
 if(!snippet)return null;
 // Supplied by us via env, not by a user, and required verbatim by Enamad.
 return <div className="mt-5 flex flex-wrap items-center gap-3 [&_img]:h-auto [&_img]:max-w-[90px]" dangerouslySetInnerHTML={{__html:snippet}}/>;
}
