...
function TeacherIntroVideo({endpoint,fa}:{endpoint:string}&Localized){
 ...
 return <Shell title={tr(fa,'ویدیوی معرفی','Introduction video')}><p className="mt-2 text-sm leading-7 text-muted">{tr(fa,'یک ویدیوی MP4، WebM یا MOV حداکثر ۵۰ مگابایت بارگذاری کنید. با ثبت فایل جدید، ویدیوی قبلی جایگزین می‌شود.','Upload an MP4, WebM, or MOV video up to 50 MB. A new upload replaces the previous video.')}</p><p className="mt-3 rounded-xl bg-[#f5f6fa] p-3 text-sm">{application.data?.introVideoKey?tr(fa,'ویدیوی معرفی ثبت شده است.','An introduction video is saved.'):tr(fa,'هنوز ویدیویی ثبت نشده است.','No introduction video has been uploaded.')}</p>{preview.data?.url&&<div className="mt-4 overflow-hidden rounded-2xl border hairline"><video controls preload="metadata" className="max-h-96 w-full bg-black" src={preview.data.url}/><div className="flex items-center gap-3 bg-slate-100 p-4"><span className="text-xs font-bold text-slate-500">01:23</span><div className="relative h-2 flex-1 rounded-full bg-slate-300"><div className="absolute inset-y-0 start-0 w-1/3 rounded-full bg-blue"></div></div><span className="text-xs font-bold text-slate-500">05:00</span></div></div>}<form className="mt-4" onSubmit={async event=>{event.preventDefault();const element=event.currentTarget,form=new FormData(element),file=form.get('file');if(!(file instanceof File)||!file.size)return;setBusy(true);setError('');try{const fileId=await uploadFile(file,'teacher-intro-video',fa);await action.mutateAsync(()=>api('/teacher/profile/intro-video',{method:'PUT',body:JSON.stringify({fileId})}));element.reset()}catch(reason){setError(apiMessage(reason,tr(fa,'بارگذاری ویدئو ناموفق بود.','Video upload failed.')))}finally{setBusy(false)}}}><input name="file" type="file" accept=".mp4,.webm,.mov" required className="w-full rounded-xl border hairline p-3"/><Submit fa={fa} busy={busy||action.isPending}>{tr(fa,'ذخیره ویدیوی معرفی','Save introduction video')}</Submit></form>{error&&<p role="alert" className="mt-3 rounded-xl bg-red-50 p-3 text-red-800">{error}</p>}<Status fa={fa} error={action.error} ok={action.isSuccess}/></Shell>
}
