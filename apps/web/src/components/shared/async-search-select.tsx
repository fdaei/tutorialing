...
export function AsyncSearchSelect({ entity, value, onChange, placeholder, disabled, className = '', name }: Props) {
  ...
  const result = useInfiniteQuery({
    queryKey: ['async-select', entity, query],
    initialPageParam: 1,
    queryFn: ({ pageParam }) =>
      api<ItemsPage<Option>>(`/search/${entity}?q=${encodeURIComponent(query)}&page=${pageParam}&pageSize=20`),
    getNextPageParam: (last) => (last.pagination.hasMore ? last.pagination.page + 1 : undefined),
    enabled: open && !disabled,
    staleTime: 30_000,
  });
  const options = useMemo(() => result.data?.pages.flatMap((page) => page.items) ?? [], [result.data]);
  ...
  return (
    <div ref={root} className={`relative ${className}`} onKeyDown={keyDown}>
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="input flex w-full items-center gap-3 text-start disabled:opacity-60"
      >
      ...
      {open && (
        <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-2xl border hairline bg-white shadow-2xl">
          <div className="border-b hairline p-3">
            <div className="flex items-center gap-2 rounded-xl bg-[#f5f6fa] px-3">
              <Search size={16} />
              <input
                autoFocus
                value={input}
          ...
          <div id={`${id}-list`} role="listbox" className="max-h-72 overflow-auto p-2">
            {result.isLoading && (
            ...
            {options.map((option, index) => (
              <button
                ...
                className={`flex w-full items-start gap-3 rounded-xl p-3 text-start ${index === active ? 'bg-[#eef2ff]' : ''}`}
              >
                <span className="min-w-0 flex-1">
                  <strong className="block truncate text-sm">{option.label}</strong>
                  {option.description && <small className="mt-1 block truncate text-muted">{option.description}</small>}
                </span>
                {value?.id === option.id && <Check size={17} className="text-blue" />}
              </button>
            ))}
            {result.hasNextPage && (
              <button
