...
export function JalaliDateTimePicker({
  ...
  const selected = value ?? null,
    initial = toJalali(selected ?? new Date());
  const [open, setOpen] = useState(false),
    [view, setView] = useState({ jy: initial.jy, jm: initial.jm }),
    [hour, setHour] = useState(String(selected?.getHours() ?? 9).padStart(2, '0')),
    [minute, setMinute] = useState(String(selected?.getMinutes() ?? 0).padStart(2, '0'));
  useEffect(() => {
    if (!value) return;
  ...
  function pick(jd: number) {
    const g = toGregorian(view.jy, view.jm, jd),
      next = new Date(g.gy, g.gm - 1, g.gd, withTime ? clampHour(hour) : 12, withTime ? clampMinute(minute) : 0, 0, 0);
  ...
  function applyTime() {
    if (!selectedJ) {
      pick(Math.max(1, toJalali(new Date()).jd));
      return;
    }
    const g = toGregorian(selectedJ.jy, selectedJ.jm, selectedJ.jd),
      next = new Date(g.gy, g.gm - 1, g.gd, clampHour(hour), clampMinute(minute), 0, 0);
    if (startOfDay(next) >= min) onChange(next);
    setOpen(false);
  }
  return (
    <div ref={root} className={`relative ${className}`} dir={translate(locale, 'supportmyTicketManagerLtr')}>
      ...
      <button
        ...
        aria-expanded={open}
        className="input flex w-full items-center gap-3 text-start disabled:opacity-50"
      >
            ...
            {days.map((day, index) =>
              day === null ? (
                ...
                <DayButton
                  key={day}
                  day={day}
                  disabled={startOfDay(fromJalali(view.jy, view.jm, day)) < min}
                  selected={!!selectedJ && selectedJ.jy === view.jy && selectedJ.jm === view.jm && selectedJ.jd === day}
                  today={sameDate(fromJalali(view.jy, view.jm, day), today)}
                  onClick={() => pick(day)}
                  fa={fa}
                />
              ),
            ...
            <div className="mt-4 border-t hairline pt-4">
              ...
              <div dir="ltr" className="flex items-center gap-2">
                <input
                  ...
                  onBlur={() => setHour(String(clampHour(hour)).padStart(2, '0'))}
                ...
                <input
                  ...
                  onBlur={() => setMinute(String(clampMinute(minute)).padStart(2, '0'))}
                  className="input min-w-0 flex-1 text-center"
                />
                <button
                  type="button"
...
function monthGrid(jy: number, jm: number) {
  const first = fromJalali(jy, jm, 1),
    offset = (first.getDay() + 1) % 7,
    length = jm <= 6 ? 31 : jm <= 11 ? 30 : isLeapJalali(jy) ? 30 : 29;
  return [...Array(offset).fill(null), ...Array.from({ length }, (_, i) => i + 1)] as Array<number | null>;
}
function fromJalali(jy: number, jm: number, jd: number) {
  const g = toGregorian(jy, jm, jd);
  return new Date(g.gy, g.gm - 1, g.gd, 12);
}
function isLeapJalali(jy: number) {
...
function d2j(jdn: number): Jalali {
  ...
  if (k >= 0) {
    ...
    const jy2 = jy - 1,
      k2 = k + 179 + (jalCal(jy2).leap === 1 ? 1 : 0);
    return { jy: jy2, jm: 7 + div(k2, 30), jd: mod(k2, 30) + 1 };
  }
