  ...
  return (
    <LocaleProvider locale="fa">
      <JalaliDateTimePicker
        ...
        value={value}
        onChange={setValue}
        minDate={new Date('2026-07-19T00:00:00')}
      />
    </LocaleProvider>
  );
}
...
describe('JalaliDateTimePicker', () => {
  ...
  it('opens a real RTL Jalali calendar and stores the selected value as ISO UTC', () => {
    ...
    const selectableDay = Array.from(screen.getByRole('dialog').querySelectorAll('button')).find(
      (button) => !button.disabled && /^\d|^[۰-۹]+$/.test(button.textContent ?? ''),
