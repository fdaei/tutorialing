  ...
  return (
    <QueryClientProvider client={client}>
      <LocaleProvider locale="en">
        <AsyncSearchSelect entity="users" value={value} onChange={setValue} name="userId" />
      </LocaleProvider>
    </QueryClientProvider>
  );
}
...
describe('AsyncSearchSelect', () => {
  beforeEach(() => {
    ...
    global.fetch = jest.fn().mockResolvedValue({
      ...
      status: 200,
      json: async () => ({
        items: [{ id: 'user-1', label: 'Sara Dadkhah', description: '09120000001' }],
        pagination: { page: 1, pageSize: 20, total: 1, pages: 1, hasMore: false },
      }),
    }) as jest.Mock;
  });
  ...
  it('debounces server search and selects a readable label without exposing the technical id', async () => {
    ...
    await waitFor(
      () =>
        expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/search/users?q=Sara'), expect.any(Object)),
      { timeout: 1500 },
    );
