...
export const walletService={
 ...
 getTransactions:async():Promise<Transaction[]>=>{const rows=await api<Array<{id:string;description:string;referenceType:string;direction:'CREDIT'|'DEBIT';amount:number;balanceAfter:number;status:'SUCCESS';createdAt:string;transactionId:string}>>('/payments/wallet/transactions');return rows.map(row=>({id:row.id,title:row.description,type:row.referenceType==='Refund'?'REFUND':row.direction==='CREDIT'?'TOP_UP':'CLASS_PURCHASE',direction:row.direction,amount:row.amount,balanceAfter:row.balanceAfter,status:row.status,createdAt:row.createdAt,trackingId:row.transactionId,paymentMethod:'کیف پول'}));},
 getInvoices:async():Promise<Invoice[]>=>{const rows=await api<Array<{id:string;purpose:string;amount:number;status:string;createdAt:string;gatewayReference?:string}>>('/payments/invoices');return rows.map(row=>({id:row.id,number:row.gatewayReference??row.id,title:row.purpose==='BOOKING'?'پرداخت کلاس':'پرداخت بسته آموزشی',amount:row.amount,status:row.status,createdAt:row.createdAt}));},
