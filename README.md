...
سپس فرانت‌اند، API و سرویس‌های وابسته را با یک فرمان اجرا کنید:
...
> `npm run dev` در اولین اجرا ممکن است چند دقیقه زمان ببرد؛ این دستور
...
برای عیب‌یابی یا اجرای جداگانه، `npm run dev:api` بک‌اند و
`npm run dev:web` فقط فرانت‌اند را اجرا می‌کنند.
...
شماره را در فرم ورود (`/auth`) به همین شکل و بدون پیش‌شمارهٔ کشور وارد کنید؛ فرم
خودش صفر ابتدایی را برمی‌دارد و `+98` را اضافه می‌کند. در دیتابیس هم همین فرم
E.164 ذخیره می‌شود (`+989120000000`)، چون مسیر OTP فقط این قالب را می‌پذیرد.
...
| `npm run dev` | آماده‌سازی و اجرای هم‌زمان وب و API در حالت توسعه |
| `npm run dev:web` | آماده‌سازی اولیه و اجرای فقط وب در حالت توسعه |
