...
async function scenarioDuplicateGatewayRedirect(call, teacherId) {
  ...
  if (distinct.size === 2) {
    ...
    if (notFoundCount === 1 && okCount === 1) {
      console.log(
        '  FIN-002 REPRODUCED: one of the two ZarinPal authorities issued for this single payment is permanently orphaned (404 on callback) because the second gatewayRedirect call overwrote payment.authority in place.',
      );
    } else {
      console.log(
        `  Unexpected outcome shape (notFound=${notFoundCount}, ok=${okCount}) — recording as-is rather than asserting a specific pattern, since dev-fallback timing can vary run to run.`,
      );
    }
  } else {
    console.log(
      '  NOT REPRODUCED this run: only one distinct authority was issued (fix may already be applied, or a lock closed the race) — treat as good news, re-run to confirm under tighter concurrency if needed',
    );
  }
...
async function scenarioDuplicateCallbackStorm(call, teacherId, concurrency = 10) {
  ...
  console.log(
    `  ${concurrency} concurrent callbacks -> status codes: ${statuses.join(',')} (${ok} ok, ${conflict} write-conflict 409, ${serverErrors} 5xx)`,
  );
  ...
  console.log(
    `  wallet/ledger entries touching this payment after the storm: ${JSON.stringify(related.map((t) => ({ type: t.type, amount: t.amount, idempotencyKey: t.idempotencyKey })))}`,
  );
...
async function scenarioConcurrentSameSlotBooking(callA, callB, teacherId) {
  ...
  console.log(
    '  PASS: exactly one concurrent booking for the identical slot succeeded, the other was correctly rejected',
  );
}
